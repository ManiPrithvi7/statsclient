...
3. **Submit Credentials**:
   ...
   **Note**: The `Authorization: Bearer <token>` header is **OPTIONAL**. 
   - If provided, the Bearer token will be stored for future authenticated API calls to the server on behalf of the user.
   - If omitted, provisioning will still succeed, but the device won't have a Bearer token for future API calls.
   - The Bearer token is generated by the backend and passed through the frontend to the device.
...
Submits WiFi credentials and provisioning token. Optionally accepts Bearer token for future authenticated API calls.

**Request Headers:**
- `Content-Type: application/json`
- `Authorization: Bearer <bearer_token>` - (OPTIONAL) Bearer token for ESP32 to use for authenticated API calls to the backend server
...
**Note**: 
- The Bearer token in the Authorization header is **OPTIONAL**. Provisioning will succeed without it.
- If provided, the Bearer token is stored separately from the provisioning_token.
- The `provisioning_token` is used for CSR signing (required).
- The Bearer token (if provided) is stored for future authenticated API calls to the server on behalf of the user.
